...
app.get("/api/careernet/proxy", async (req: Request, res: Response) => {
  try {
    const { apiKey, svcType, svcCode, contentType = "json", gubun, searchTitle, searchJobNm, thisPage = "1", perPage = "20" } = req.query;

    const keyToUse = (apiKey as string) || process.env.CAREERNET_API_KEY || "";
    if (!keyToUse) {
    ...
    if (svcType) params.append("svcType", svcType as string);
    if (svcCode) params.append("svcCode", svcCode as string);
    params.append("contentType", (contentType as string) || "json");
    if (gubun) params.append("gubun", gubun as string);
    if (searchTitle) params.append("searchTitle", searchTitle as string);
    if (searchJobNm) params.append("searchJobNm", searchJobNm as string);
    params.append("thisPage", thisPage as string);
